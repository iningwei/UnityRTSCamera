    ...
    public class RTS_CameraEditor : Editor
    {
        ...
        private void MovementTab()
        {
            ...
            if (camera.limitMap)
            {
                ...
                camera.limitXMin = EditorGUILayout.FloatField("Limit XMin: ", camera.limitXMin);
                camera.limitXMax = EditorGUILayout.FloatField("Limit XMax: ", camera.limitXMax);
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                camera.limitYMin = EditorGUILayout.FloatField("Limit YMin: ", camera.limitYMin);
                camera.limitYMax = EditorGUILayout.FloatField("Limit YMax: ", camera.limitYMax);
                EditorGUILayout.EndHorizontal();
            }
        ...
        private void HeightTab()
        {
            using (new HorizontalBlock())
            {
                GUILayout.Label("Init with origin height: ", EditorStyles.boldLabel, GUILayout.Width(170f));
                camera.initWithOriginHeight = EditorGUILayout.Toggle(camera.initWithOriginHeight);
            }
